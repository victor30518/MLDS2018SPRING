...
if DICT_FILE:
    ...
    #share_lang.n_words = len(share_lang.word2index.keys())
    #temp = {i[1]:i[0] for i in share_lang.word2index.items()}
...
decoder = LuongAttnDecoderRNN(attn_model, hidden_size, share_lang.n_words, n_layers, dropout=dropout)
...
def evaluate_test(input_sentences, outputfilename):
    with open(outputfilename, 'w',encoding = 'utf-8') as f:
        for idx, sen in enumerate(input_sentences):
            #print('input:', sen)
            output_words, _ = evaluate(sen.strip())
            res = ''.join(output_words) 
            #print('output:', res)
            f.write(res)
            #if idx < len(input_sentences)-1:
            f.write('\n')
